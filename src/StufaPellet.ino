...
uint32_t wifiTime, updateSinric, updateTempTime, checkTempTime; 
String ssid, pswd;
int32_t telegramId;

bool isOnline, onlineAtBoot = false;
bool alexaCmd = false;
...
void loop() {     
  ...
  if(WiFi.isConnected()) {      
    ...
    if (millis() -updateSinric > 5000){
      updateSinric = millis();
      updateAlexa();    
    }
  // Not connected
  } else if(millis() - wifiTime > 500){
    wifiTime = millis(); 
    digitalWrite(LED_BUILTIN, !digitalRead(LED_BUILTIN));
    if(!onlineAtBoot ){
      startConnections(5000) ; 
    }
  }
  
  // Update temperature and check if new Telegram message is present
  ...
  // Check if the stoove is started ad requested
  if((millis() - checkTempTime > CHECK_ON_TIME) && (cmdMsg.request == START)){    
    ...
    // Something wrong, temperature of water is below the setpoint
    else{
  ...
  // Confimr message received from Telegram
